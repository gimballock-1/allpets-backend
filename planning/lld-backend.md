...
> - Ingress pattern (Cloudflare DNS + Traefik + cert-manager **DNS-01**) → [`deploy/k8s/ingress/README.md`](../deploy/k8s/ingress/README.md) (3.4)
...
>
> **The pivot in one line (2026-06-17):** **Payload CMS is gone.** Marketing content is now **file-based** in the Next.js repo (typed / MDX in `allpets-frontend`) — not a backend concern. The custom backend is a **Spring Boot service** in `allpets-backend`, on its own host `api.allpets.skpodduturi.dev`, reached by the site's **server-side `/api` proxy** (cross-origin; the browser stays same-origin — Frontend LLD §4.2); it owns the **`appdb`** database (role **`app_svc`**, renamed from the old `payload`) and is the **seed of the phase-2 application** (pet profiles + multi-tenancy — §12).
>
> **Inherited non-uses (HLD *Scope guardrails* topic, restated so a grepping reviewer finds only disclaimers):** **no Payload / headless CMS** (marketing content is file-based in `allpets-frontend`); **Cloudflare is the DNS provider only** — DNS-only / gray-cloud mode, **not** a reverse proxy, **not** a Tunnel, **not** Cloudflare Access (Cloudflare-as-DNS != Cloudflare-Access/Tunnel); **DNS-01 not HTTP-01**; **plain Postgres `Deployment`, not CloudNativePG**; no sealed-secrets/SOPS/external-secrets in phase 1; no GitOps (push-based CD); **no off-site backup** (local-only `pg_dump`); no NodePort (data tier is `ClusterIP` only); admin/auth surfaces **app-auth-only** (no Traefik auth middleware); **pet profiles / multi-tenancy are not built in phase 1** (seeded only — §12). This LLD must never contradict those.
...
|---|---|---|---|---|
| **Spring Boot backend** (Gradle/Kotlin-DSL, **Java 25 LTS**, **Spring Boot 3.x**, Spring Data JPA + Hibernate, Flyway, Actuator) — code **we write** | `ContactSubmission` (contact-form inbox), the Google-reviews cache (10.4), transactional email (Epic 13), health/metrics (Actuator). **Seed of phase-2** (§12). | `api.allpets.skpodduturi.dev` :8080 (its **own** origin) | `appdb` DB (5432) + MinIO `allpets-media` (9000, **future** — reserved, unused in phase 1) | new (this pivot) / 10 / 13 |
| **Cal.com self-hosted** (no EE) — a **product we deploy**, not code we write | Bookings, vet schedules, event types, intake answers, Google-Calendar OAuth tokens, reminder workflows | `book.allpets.skpodduturi.dev` :3000 | `calcom` DB (5432) | 6 |
| **Plausible CE** — a **product we deploy** | Cookieless site analytics + dashboard + tracking script | `analytics.allpets.skpodduturi.dev` :8000 | Plausible meta in Postgres (5432) + ClickHouse events (8123) | 11 |
...
> Solid arrows are the live request/data path; dashed arrows are out-of-band, embedded, or **reserved-but-unused** (MinIO). The site→API path is **public and cross-origin** (the site's server-side `/api` proxy calls `api.allpets.skpodduturi.dev` through Traefik; the browser stays same-origin), gated by the Spring CORS allowlist — there is **no** in-cluster frontend→backend call (the former Payload `allow-from-frontend` content-API seam is removed by the pivot). The only ingress into `allpets-database` is from `allpets-backend` (HLD *NetworkPolicies* + *data-tier-only-from-backend* decision topics).
...
- **SPF/DKIM/DMARC** live in Cloudflare DNS (Epic 13 13.3) — DNS records managed in the Cloudflare dashboard (or via the Cloudflare API), not backend code; one correct record set covers both senders because they share the `From:` domain. **Bounce/complaint webhook handling (13.8, P2)** would land as a small unauthenticated `POST` endpoint in the Spring backend (signature-verified, exempt from CSRF/rate-limit by path) if/when built — not in the phase-1 critical path.
...
- **Host:** `book.allpets.skpodduturi.dev` — its **own dedicated origin** (never a path under `allpets`) because Cal.com needs a stable host for **cookies + Google OAuth callbacks** (HLD *Overview / hosts* topic, req §9). The Spring API now gets its own host for the same reasons (§8 / HLD *DNS & TLS* topic).
...
|---|---|---|
| `NEXT_PUBLIC_WEBAPP_URL` | build-time, plain (repo **variable**) | `https://book.allpets.skpodduturi.dev` — **inlined by Next at build**, cannot be overridden at runtime (6.2). Build-arg in 15.5. |
...
|---|---|---|---|---|---|
| **Spring backend** | `allpets-api` / `api`, port **8080**, RollingUpdate `maxUnavailable:0/maxSurge:1` | `allpets-api` :80 → 8080 | **`api.allpets.skpodduturi.dev`** → **`api-allpets-skpodduturi-dev-tls`** (Ingress in **this repo**, `allpets-backend` ns) | liveness `/actuator/health/liveness`; readiness `/actuator/health/readiness` (DB-backed) | 1 (raise w/ headroom) |
| Cal.com (6.3) | `calcom` / `calcom`, port 3000 | `calcom` :80 → 3000 | `book.allpets.skpodduturi.dev` → `book-allpets-skpodduturi-dev-tls` (this repo) | Cal.com built-in health path, generous `initialDelaySeconds` | 1 |
| Plausible (11.1) | `plausible` / `plausible`, port 8000 | `plausible` :80 → 8000 | `analytics.allpets.skpodduturi.dev` → `analytics-allpets-skpodduturi-dev-tls` (this repo) | Plausible built-in | 1 |

- **The new API host (pivot).** `api.allpets.skpodduturi.dev` is a **net-new** public host: a **Cloudflare** DNS-only / **gray-cloud** (proxy **off**) A-record → `50.35.125.239` (TTL 300) so Traefik + cert-manager + the 308 redirect behave exactly as before, its **own Traefik Ingress + cert-manager DNS-01 cert** `api-allpets-skpodduturi-dev-tls` in `allpets-backend`, fronting the Spring `Service` on **:8080**. The site's server-side `/api` proxy calls it **cross-origin** (the browser stays same-origin — Frontend LLD §4.2; HLD *DNS & TLS* + *Spring-backend location* topics). This replaces the former `allpets.skpodduturi.dev/admin` routing of Payload — the Spring API is **not** served as a path under the site (the site's `/api/*` proxy is a Next.js route handler, not a Traefik ingress path to the backend).
- **CORS (Spring side, not ingress).** Cross-origin access is controlled by the **Spring app's CORS config** (`config` package), **not** a Traefik middleware. The allowlist is exactly **`https://allpets.skpodduturi.dev`** (the site origin; plus localhost for dev) — **never `*`** in production — with the methods/headers `POST /contact` + `GET /reviews` need and credentials **off** in phase 1. Keeping the origin boundary in the app avoids adding the cluster's first cross-namespace Traefik middleware (HLD *DNS & TLS* topic, Epic 10 10.6).
- **Ingress rules (3.4, non-negotiable):** `ingressClassName: traefik`; annotation `cert-manager.io/cluster-issuer: letsencrypt-cloudflare` (the **new dedicated** allpets Cloudflare DNS-01 issuer — see the TLS bullet; **not** the shared cluster-wide `letsencrypt-prod`); `spec.tls.secretName = <host>-tls`; single `host` rule, `path: /`, `pathType: Prefix`; the per-namespace `redirect-https` Middleware annotation (`<namespace>-redirect-https@kubernetescrd`, a **308**, per-namespace because `allowCrossNamespace` is OFF). The Ingress lives in the **same namespace as its Service** — `api`, `book`, and `analytics` are all in this repo's `allpets-backend` namespace and **share one** `allpets-backend` `redirect-https` Middleware. The site's own Ingress (`allpets.skpodduturi.dev`) is in the **frontend** repo.
...
- **api/calcom/plausible ↔ 3.4 + 14.6 + 15.x** — ingress template (incl. the **new `api.allpets.skpodduturi.dev`** Ingress + DNS-01 cert), secrets materialization, CI/CD `:sha` pinning + Cal.com migrate-gate.
...
- Ingress pattern: [`deploy/k8s/ingress/README.md`](../deploy/k8s/ingress/README.md) (3.4) — host→namespace→TLS map (incl. the new `api.allpets.skpodduturi.dev`), DNS-01, per-namespace `redirect-https`, app-auth-only fold-in.
