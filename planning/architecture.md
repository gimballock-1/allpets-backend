...
- The Next.js site does **not** call the Spring backend over the in-cluster `allow-from-frontend` path. The pivot makes the API a **first-class public host** (`api.allpets.kinvee.in`); the browser calls it **cross-origin** over the public-ingress plane (CORS allowlisted for `https://allpets.kinvee.in`). The former same-origin `/api` routing seam is **gone**.
...
flowchart TB
    ...
    visitor -->|"cross-origin fetch to api.allpets.kinvee.in (CORS allowlist)"| traefik
    api -->|"appdb :5432"| pg
...
|---|---|---|---|---|
| **Next.js marketing site** | Public site (Home, Services, About, Contact, legal). App Router, SSR/SSG, Cal.com embed on `/book` and service CTAs. **Marketing content is file-based** (typed / MDX files committed in `allpets-frontend`). | `allpets.kinvee.in` :3000 | `allpets-frontend` | Content ships **inside the site image** (no content API, no CMS). Calls the **Spring API cross-origin** at `api.allpets.kinvee.in` for contact-form POST + reviews; **does not** touch Postgres/MinIO directly. |
...
|---|---|---|---|
| `appdb` | `app_svc` | `contact_submission` (Contact-form inbox), the Google-reviews cache (10.4), and **later** pet/patient profiles + records (phase 2, §11). | The **Spring backend (ns `allpets-backend`)** — the only client of this DB. The Next.js site never queries it; the browser calls the Spring API cross-origin. |
...
> **The Spring backend runs in `allpets-backend`** (reconciled to the deployed manifests — see §1). It is reached by the browser over its **own public host** `api.allpets.kinvee.in` (cross-origin), and it is the workload from which all `appdb` + MinIO access originates — permitted by `allow-from-backend`. There is **no Payload** in this topology anymore.
...
> **The browser→API path is public, not in-cluster.** Because the Spring API is its own public host (`api.allpets.kinvee.in`), the Next.js site's calls to it go out to the browser and back **through Traefik** (`allow-traefik-ingress`, :8080), not over a frontend→backend in-cluster path. The former `allow-from-frontend` content-API seam is **removed by the pivot** — the site no longer makes a server-side in-cluster call to the backend for content (content is file-based). If a future SSR-side server-to-server call is ever wanted, it would be re-introduced explicitly; phase 1 does not need it.
>
> **Why there is no frontend→database allow.** The Next.js site never opens a Postgres/MinIO connection: its marketing content is **file-based** (shipped in its image), and the only dynamic data (contact POST, reviews) comes from the **Spring API cross-origin** over the public host. The Spring backend — running in `allpets-backend` — is the sole client of `appdb` and MinIO (covered by `allow-from-backend`). Keeping the data tier reachable **only** from `allpets-backend` is the deployed, verified posture; opening it to the frontend would widen the blast radius for no functional gain.
>
...
The deployed `allow-from-backend` policy is the **sole** ingress into `allpets-database` (Postgres 5432 / MinIO 9000 / ClickHouse 8123). All DB/object-store clients — **Spring backend, Cal.com, Plausible** — run in `allpets-backend`, so **no `allpets-frontend → allpets-database` allow exists or is needed**. The Next.js site ships file-based content and calls the Spring API cross-origin; the Spring backend (backend) is the only client of `appdb` + MinIO. This keeps the data tier's blast radius to one namespace and matches the verified cluster behavior (frontend pod refused on :5432, backend pod succeeds).

### 13.8 **Drop Payload CMS → file-based marketing content** — pivot 2026-06-17 (supersedes the Payload-centric baseline)
Payload CMS is **removed entirely**: no CMS process, no `payload` content database, no content admin UI. Marketing content is now **file-based** — typed / MDX content files committed in the `allpets-frontend` repo, **set-and-forget**, edited by the operator via a **Git commit + auto-deploy**. Rationale: a single-location clinic's marketing copy changes rarely; a CMS + its database + its admin surface is operational and security overhead the project doesn't need, and "edit a file, push, it deploys" is simpler for a solo operator than running and securing a CMS. Trade-off: content edits require a Git commit / redeploy rather than a live admin UI (acceptable for set-and-forget content). The former `payload` content DB is dropped; the `contact_submission` + reviews-cache data moves to the new Spring app DB (13.9).
...
The Spring backend is exposed on its **own public host** `api.allpets.kinvee.in` — a new Route 53 A-record → `50.35.125.239`, its **own Traefik Ingress + cert-manager DNS-01 cert** (`api-allpets-kinvee-in-tls`) in `allpets-backend`. The Next.js site calls it **cross-origin**; the API's **CORS allowlist** permits only `https://allpets.kinvee.in`. Rationale: a clean, independent origin (own cert, own CORS, no path-routing entanglement with the site) — matching how Cal.com gets its own host. **This removes the former same-origin `/api` routing seam** and the in-cluster `allow-from-frontend` content-API call.
...
- **Same-origin `/api` seam / in-cluster frontend→backend content API** — removed by the pivot; the site calls the Spring API **cross-origin** on `api.allpets.kinvee.in` (13.10).
