...
| **cert-manager** | Issues + auto-renews Let's Encrypt certs via a dedicated `letsencrypt-cloudflare` ClusterIssuer, **DNS-01 solver (Cloudflare)**. | n/a | `cert-manager` (shared) | Writes `_acme-challenge` TXT into the Cloudflare `skpodduturi.dev` zone, authenticated by the `cloudflare-api-token-secret` secret. |
